...
// مسیر وردپرس
$wp_path = __DIR__ . '/wp-load.php';
if (!file_exists($wp_path)) {
    echo json_encode(['error' => 'WordPress not found at: ' . $wp_path]);
    exit();
}
...
if ($action === 'track_order') {
    $code = sanitize_text_field($data['tracking_code'] ?? '');
    
    if (empty($code)) {
        echo json_encode(['found' => false, 'error' => 'کد الزامی است']);
        exit();
    }
    
    // جستجو در سفارشات
    $order = $wpdb->get_row($wpdb->prepare(
        "SELECT ID, post_date, post_status FROM {$wpdb->posts} 
         WHERE post_type = 'shop_order' 
         AND (ID = %d OR post_title LIKE %s)
         ORDER BY ID DESC LIMIT 1",
        intval($code), '%' . $wpdb->esc_like($code) . '%'
    ));
    
    if (!$order) {
        echo json_encode(['found' => false, 'message' => 'سفارش یافت نشد: ' . $code]);
        exit();
    }
    ...
    $customer = trim($first_name . ' ' . $last_name) ?: 'مشتری';
    ...
    $status = $status_map[$order->post_status] ?? $order->post_status;
    ...
    echo json_encode([
        ...
        'order' => [
            ...
            'status' => $status,
            'total' => get_post_meta($order->ID, '_order_total', true) ?: '0',
            'payment' => get_post_meta($order->ID, '_payment_method_title', true) ?: 'آنلاین',
            'items' => $item_names,
    ...
    if ($query->have_posts()) {
        while ($query->have_posts()) {
            ...
            $products[] = [
                ...
                'url' => get_permalink(),
                'stock' => $product ? ($product->is_in_stock() ? 'موجود' : 'ناموجود') : 'نامشخص'
            ];
        }
        wp_reset_postdata();
    }
...
echo json_encode([
    ...
    'valid_actions' => ['track_order', 'search_product', 'health_check'],
    'received' => $data
]);
